...
const MAX_VARIABLES_PER_PROJECT = 1000;
...
// Brand-new keys are created only while the project is under
// MAX_VARIABLES_PER_PROJECT, so a chatty/compromised device can't grow the
// metadata table without bound; existing keys always refresh regardless.
async function upsertVariables(
  ...
  try {
    ...
    const existingRows = await env.DB
      ...
      .bind(projectId, ...due)
      .all<{ key: string }>();
    const existing = new Set(existingRows.results.map((r) => r.key));
    ...
    // Existing keys (last_seen refresh) + capped new keys (first-sight create).
