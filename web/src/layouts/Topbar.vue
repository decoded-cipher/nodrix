...
const pageTitle = computed(() => crumbs.value[crumbs.value.length - 1]?.label ?? '');
</script>

<template>
  <header class="flex h-14 items-center justify-between border-b border-neutral-200 bg-white px-5">
    <nav class="flex items-center gap-1.5 text-sm text-neutral-500">
      <template v-for="(c, i) in crumbs" :key="i">
        ...
        <svg
          ...
          stroke-width="2" stroke="currentColor" class="h-3.5 w-3.5 text-neutral-300"
        >
    ...
    <div class="flex items-center gap-2">
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        class="rounded-md p-2 text-neutral-500 hover:bg-neutral-100 hover:text-neutral-900"
        title="Docs &amp; GitHub"
      >
      ...
      <h1 v-if="pageTitle && !hasPageActions" class="ml-2 hidden text-sm text-neutral-600 sm:block">
        {{ pageTitle }}
      </h1>
    </div>
  </header>
