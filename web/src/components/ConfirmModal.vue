  ...
  <Teleport to="body">
    <Transition name="confirm-fade">
      <div
        ...
        class="fixed inset-0 z-[100] flex items-center justify-center bg-neutral-900/50 px-4"
        role="dialog"
        aria-modal="true"
        ...
        <div class="w-full max-w-md overflow-hidden rounded-xl bg-white shadow-2xl">
          <div class="flex items-start gap-3 px-5 py-4">
            ...
            <div
              ...
              :class="danger ? 'bg-red-100 text-red-600' : 'bg-neutral-100 text-neutral-600'"
            >
              <svg
                v-if="danger"
            ...
            <div class="min-w-0 flex-1">
              <h2 class="text-sm font-semibold text-neutral-900">{{ opts.title }}</h2>
              <p v-if="opts.message" class="mt-1 whitespace-pre-line text-xs text-neutral-600">
                {{ opts.message }}
              </p>
              <ul
                v-if="opts.details && opts.details.length"
                class="mt-2 space-y-1 text-xs text-neutral-700"
              >
                <li v-for="(d, i) in opts.details" :key="i" class="flex gap-2">
                  <span class="mt-1 inline-block h-1 w-1 shrink-0 rounded-full bg-neutral-400" />
                  <span>{{ d }}</span>
                </li>
              </ul>
            </div>
          ...
          <div class="flex items-center justify-end gap-2 border-t border-neutral-100 bg-neutral-50 px-5 py-3">
            <button
              type="button"
              class="rounded-md border border-neutral-300 bg-white px-3 py-1.5 text-xs hover:bg-neutral-100"
